...
If ("$ENV:SQL" -match 2005) { Setup-SqlServers "$ENV:SQL" /Instance=$ENV:INSTANCE }
ElseIf ("$ENV:SQL" -match 2008) { Setup-SqlServers "$ENV:SQL" /Instance=$ENV:INSTANCE /SkipRules=PerfMonCounterNotCorruptedCheck } 
ElseIf ("$ENV:SQL" -match 2012) { Setup-SqlServers "$ENV:SQL" /Instance=$ENV:INSTANCE "/SkipRules=PerfMonCounterNotCorruptedCheck FacetPowerShellCheck RebootRequiredCheck" } 
Else { Setup-SqlServers "$ENV:SQL" /Instance=$ENV:INSTANCE /SkipRules=PerfMonCounterCheck }
