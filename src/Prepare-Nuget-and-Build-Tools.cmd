...
rem    pushd MyApp1.Tests/Bin/Debug
rem    "%NUNIT_CONSOLE_RUNNER%" --labels=On --workers=1 MyApp1.Tests.dll
rem    "%REPORT_UNIT%" .\
rem    popd
...
for %%p in (vswhere NUnit.ConsoleRunner NUnit.Extension.NUnitV2Driver NUnit.Extension.NUnitV2ResultWriter ReportUnit xunit.runner.console xunit) DO (
  echo Installing package [%%p] into temp folder
...
rem Assign XUNIT_CONSOLE_RUNNER
pushd "%BUILD_TOOLS_ONLINE%"
cd xunit.runner.console*\tools
cd net*
dir /b /s xunit.console.exe > exe-name.txt
set XUNIT_CONSOLE_RUNNER_NET=
for /F %%v in (exe-name.txt) DO set XUNIT_CONSOLE_RUNNER_NET=%%v
cd .. && cd *core*2.0
dir /b /s xunit.console.dll > exe-name.txt
set XUNIT_CONSOLE_RUNNER_CORE20=
for /F %%v in (exe-name.txt) DO set XUNIT_CONSOLE_RUNNER_CORE20=%%v
popd
Echo XUNIT_CONSOLE_RUNNER_NET: [%XUNIT_CONSOLE_RUNNER_NET%]
Echo XUNIT_CONSOLE_RUNNER_CORE20: [%XUNIT_CONSOLE_RUNNER_CORE20%]

rem Assign REPORT_UNIT
pushd "%BUILD_TOOLS_ONLINE%"
cd ReportUnit*\tools
dir /b /s ReportUnit*.exe > exe-name.txt
set REPORT_UNIT=
for /F %%v in (exe-name.txt) DO set REPORT_UNIT=%%v
popd
Echo ReportUnit (var REPORT_UNIT): [%REPORT_UNIT%]
