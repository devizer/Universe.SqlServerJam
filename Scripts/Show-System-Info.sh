...
Say "IP WHO"
echo IP WHO >> "$ReportName"
curl -kSL https://ipwho.is/ | jq . | tee -a "$ReportName"
...
if [[ "$(uname -s)" == Darwin ]] || [[ "$(uname -s)" == Linux ]]; then
  ...
  Say "Installing 7Zip"
  Say --Display-As=Error "7zip choco package is broken"
  # choco install 7zip --version 21.7.0 --force --allow-downgrade --no-progress || choco install 7zip --force --allow-downgrade --no-progress
fi
...
if [[ "$(uname -s)" == Linux ]]; then
   ...
   # fio
   if [[ "$(uname -s)" == Linux ]] && [[ -z "$(command -v fio)" ]]; then
   ...
   Say "Installing for for Windows: choco install --force --allow-downgrade -y --no-progress fio"
